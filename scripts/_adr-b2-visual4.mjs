...
const IMG = 'zterm-probe-b2vis4.exe';
...
await type('herdr --session zterm-b2vis4\r', 3000);
await type('dsh-tui\r', 4000);
for (const ch of 'ab cd') await type(ch);
await sleep(400);
await val(`TabManager.switchTo(TabManager.tabs.find(x => x.tabId === '${sshTabId}').id); 'switched'`);
await sleep(600);
await val(`TabManager.getActive().term.focus(); 'focused'`);
await sleep(300);
...
await val(`ipcRenderer.send('pty-input', { tabId: '${sshTabId}', data: 'herdr --session zterm-b2vis4 kill 2>/dev/null; echo K\\r' })`);
ws.close();
process.exit(0);
