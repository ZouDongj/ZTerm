...
const swState = () => val(`(() => {
  ...
  return JSON.stringify({
    sw: snap.softwareCaret || a.instrumentation?.softwareCaret || null,
    target: snap.target,
...
await type('herdr --session zterm-adrb2\r', 3000);
await type('dsh-tui\r', 4000);
await type('ab cd');
await sleep(500);
console.log('[after typing]', await swState());
const rawNow = await val('(globalThis.__ztRawCapture || []).join("")');
const sig = (rawNow.match(/[0;38;2;d+;d+;d+;48;2;d+;d+;d+m/g) || []).length;
const units = (rawNow.match(/[?2026h/g) || []).length;
const shows = (rawNow.match(/[?25h/g) || []).length;
console.log('[live raw]', JSON.stringify({ len: rawNow.length, sig, units, shows }));
...
await type('\u0002q', 600);
await val(`ipcRenderer.send('pty-input', { tabId: '${sshTabId}', data: 'herdr --session zterm-adrb2 kill 2>/dev/null; echo K\\r' })`);
await sleep(1000);
