...
// Ctrl+Shift+E: dump terminal buffer to file (debug，不进快捷键表)
document.addEventListener('keydown', e => {
    ...
    const dumpPath = path.join(process.env.TEMP || '/tmp', `zterm-dump-${now}.txt`);
    const lines = [];
    const collect = (t) => {
        if (!t.term) return;
        try {
            const buf = t.term.buffer.active;
            lines.push(`=== cols=${t.term.cols} rows=${t.term.rows} cursor=${t.term.buffer.active.cursorX},${t.term.buffer.active.cursorY} ===`);
    ...
    if (tab.splitRoot) {
        getAllPanes(tab).forEach((p, i) => {
            lines.push(`\n─── Pane ${i+1}: ${p.id} focused=${p.focused} tabId=${p.tabId} ───`);
            collect(p);
        });
    } else {
        collect(tab);
    }
    try { fs.writeFileSync(dumpPath, lines.join('\n'), 'utf-8'); } catch(e) {}
    showToast('终端内容已导出: ' + dumpPath);
});
