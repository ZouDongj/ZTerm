...
const SFTP = {
    ...
    async navigate(path) {
        ...
        const { files, error } = await ipcRenderer.invoke('sftp-readdir', { tabId: this._tabId, path });
        if (error) {
            showToast(error, true);
            body.innerHTML = '<div class="sftp-empty">加载失败</div>';
            return;
        }
        // 目录在前，同类按名称字母序
        this._files = (files || []).sort((a, b) => {
    ...
    async download(remotePath, filename) {
        ...
        if (error) {
            ...
            if (error !== 'Transfer cancelled') showToast('下载失败: ' + error, true);
        } else {
    ...
    async uploadLocal(localPath) {
        ...
        if (error) {
            TransferManager.cancel(tid);
            if (error !== 'Transfer cancelled') showToast('上传失败: ' + error, true);
        } else {
            TransferManager.complete(tid);
            await this.refresh();
        }
    ...
    mkdir() {
        ...
        row.innerHTML = '<span class="sftp-item-icon">📁</span><input placeholder="新建目录名称，Enter 确认 / Esc 取消" style="flex:1;background:rgba(var(--accent-rgb),0.06);border:1.5px solid rgba(var(--accent-rgb),0.25);border-radius:8px;padding:4px 10px;color:#abb2bf;font-size:12.5px;font-family:inherit;outline:none">';
        ...
        input.addEventListener('keydown', async (e) => {
            ...
            if (error) showToast('创建失败: ' + error, true);
...
(() => {
    ...
    win.addEventListener('drop', e => {
        ...
        [...(e.dataTransfer.files || [])].forEach(f => {
            ...
            if (!localPath) return;
            try { if (fs.statSync(localPath).isDirectory()) { showToast('暂不支持上传文件夹: ' + f.name, true); return; } } catch(err) {}
            SFTP.uploadLocal(localPath);
        });
    });
})();
